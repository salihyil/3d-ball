...
import type { GameSnapshot, Team } from '../types';
...
export default function Game() {
  ...
  const [gameOver, setGameOver] = useState<{ score: { blue: number; red: number }; winner: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const rejoinRoom = () => {
      socket.emit('join-room', { roomId, nickname }, (res: { success: boolean; room?: { players: { id: string; team: string }[], gameState: string } }) => {
    ...
    const handleSnapshot = (snapshot: GameSnapshot) => {
      ...
      // Update my boost cooldown and team identity
    ...
    const handleGoalScored = (data: { team: Team; scorer: string; score: { blue: number; red: number } }) => {
      setGoalInfo(data);
      setScore(data.score);
  ...
  return (
    <div className="game-container">
      ...
      >
        <GameScene latestRef={latestRef} myTeam={myTeam} />
      </Canvas>
      ...
      {hostLeft && (
        <div className="gameover-overlay" style={{ zIndex: 1000 }}>
          <div className="gameover-card glass-card">
            <div className="gameover-title" style={{ color: '#ff4a4a', fontSize: '2rem' }}>Host Disconnected</div>
            <div style={{ color: 'var(--text-secondary)', marginBottom: '24px', textAlign: 'center', lineHeight: 1.5 }}>
              The host has unexpectedly left the game or their connection dropped. The room is now closed.
            </div>
            <div className="gameover-actions" style={{ justifyContent: 'center' }}>
        ...
        <div className="controls-hint-overlay">
          <div className="controls-hint-card">
            ...
            <div className="hint-row">
              <div className="hint-keys"><kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd></div>
              <span>Move player</span>
            </div>
            <div className="hint-row">
              <div className="hint-keys"><kbd className="key-wide">Spacebar</kbd></div>
